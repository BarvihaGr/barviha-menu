...
# ВАЖНО: правка этого файла вступает в силу только со СЛЕДУЮЩЕГО деплоя —
# крон запускает ту версию, что уже лежит на диске, она обновляет репозиторий
# и дорабатывает по старому тексту. Коммит, который меняет сам скрипт, нужно
# «догнать» ещё одним коммитом (2026-09-07: так первый раз не запустились
# content-migrations, пока не пришёл следующий пуш).
#
