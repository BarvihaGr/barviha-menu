...
/**
 * Гладкие округлые силуэты спилов «по обводке»: вытянутые овалы без
 * единого острого угла — как настоящий срез ствола. Пропорции взяты с
 * зелёной обводки: левый — яйцо (уже сверху), центр — полнее и круглее,
 * правый — самый вытянутый и слегка наклонён.
 *   ax,by — полуоси; tn — мягкое сужение кверху; rot — наклон (°); ph — фаза.
 */
const BLOB = [
  { ax: 35, by: 51, tn: 0.18, rot: -2, ph: 0.6 }, // слева — яйцо
  { ax: 40, by: 51, tn: 0.08, rot: 0, ph: 2.1 }, // центр — полнее/круглее
  { ax: 31, by: 55, tn: 0.1, rot: 6, ph: 4.0 }, // справа — вытянутый, с наклоном
] as const;

const VBC = 65; // центр кадра по X
const VBCY = 63; // центр кадра по Y
...
function buildSlice(seed: number, shape: number): Slice {
  ...
  // округлый овал + мягкая органика (низкие гармоники, малые амплитуды) —
  // ни одного острого угла. Сужение кверху делает «яйцо».
  const cfg = BLOB[shape] ?? BLOB[0];
  const rotR = (cfg.rot * Math.PI) / 180;
  ...
  for (let i = 0; i < N; i++) {
    ...
    const noise =
      1 +
      0.05 * Math.sin(a + cfg.ph) +
      0.038 * Math.sin(2 * a + cfg.ph * 1.7) +
      0.022 * Math.sin(3 * a + cfg.ph * 2.3);
    const taperX = 1 - cfg.tn * 0.5 * (1 - Math.sin(a)); // уже сверху
    const dx = cfg.ax * Math.cos(a) * noise * taperX;
    const dy = cfg.by * Math.sin(a) * noise;
    B.push([VBC + dx * cosR - dy * sinR, VBCY + dx * sinR + dy * cosR]);
  }
