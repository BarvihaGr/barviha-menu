...
export function CategoryCard({ href, title, subtitle, icon, count, index = 0, large }: Props) {
  return (
    ...
    >
      ...
      >
        <div className="relative flex h-full flex-col items-center justify-center gap-2 bg-gradient-to-br from-[#453324] to-[#2A1B11] p-4">
          ...
          {typeof count === 'number' && (
            <span className="absolute bottom-2.5 right-2.5 text-[9px] tracking-[0.2em] text-muted">
              {count}
            </span>
          )}
        </div>
