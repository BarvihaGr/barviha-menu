...
/**
 * Экран ввода кода доступа к тестовой Киевской — см. middleware.ts: без
 * валидной cookie любой запрос на /{locale}/kievskaia* сюда и попадает,
 * даже если пришли по прямой ссылке. См. arka-gate/page.tsx (тот же паттерн).
 */
...
function KievskaiaGateForm() {
  const params = useParams<{ locale: string }>();
  const searchParams = useSearchParams();
  const next = searchParams.get('next') || `/${params.locale}/kievskaia`;
  ...
  const submit = async (value: string) => {
    ...
    setError(false);
    try {
      const res = await fetch('/api/kievskaia-gate', {
        method: 'POST',
...
export default function KievskaiaGatePage() {
  return (
