...
/**
 * Экран ввода кода доступа к «рабочим» локациям-клонам (см. middleware.ts +
 * @barviha/db WORKING_SLUGS) — общий пароль на все 25, отдельный от Арки
 * (см. /arka-gate). После верного кода — жёсткий редирект на `next`, чтобы
 * middleware сразу увидел свежую cookie.
 */
...
function TestLocGateForm() {
  ...
  const submit = async (value: string) => {
    ...
    setError(false);
    try {
      const res = await fetch('/api/test-loc-gate', {
        method: 'POST',
...
export default function TestLocGatePage() {
  return (
